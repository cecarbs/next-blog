...
const index = ({ blogs }) => {
  const title = "Personal Development Blogs | Charles Carbonel";
  const metaDescription = "Category: Personal Development";
  return (
    <>
      <Head>
        <title>{title}</title>
...
export async function getStaticProps() {
  const blogs = await sanityClient.fetch(
    `*[_type == 'post' && categories[0]->title == 'Personal Development'] | order(_createdAt desc){
      ${blogCardFields}
    }`
  );
